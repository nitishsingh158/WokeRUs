...
LendingClub is a company that enables people looking to apply for unsecured personal loans by supplying details about themselves and the loans that they would like to request (between 1K-40K). Based on the borrower’s credit score, credit history, employment history, desired loan amount and the borrower’s debt-to-income ratio, LendingClub determined whether the borrower was creditworthy and allowed the loan or not. The team at WokeRUs looked to investigate if it was possible to predict the approval of any loan based upon the features listed.
...
- The two datasets were merged with only the required columns included in a consolidated dataset.
...
Resources:
- Also please refer to our requirements.txt file for libraries and installations.
...
- Since our datasets were classified into two main categories one being accepted for a loan and the other rejection, we did analyze the columns (a run-through) to find relational contents of both datasets.
...
- In a further exploratory analysis, cleaning data was carried out where most of our features were having characters paired with numerics.
...
- Hence, our main raw or original dataset is still available locally to ensure we are using our resources to their maximum potential. (All original dataset is available in Kaggle)
...
The following algorithm was selected mainly of two key elements after a trial with various algorithms:
...
The following key takeaways bought about a model with an r2 score of 0.98 and mean square error of 1.00:
...
3. There was a lot learned, practised and implemented during this collaboration.
